...
- **Autenticacao de deploy (WIF/OIDC keyless, 2026-06-26):** o job de deploy usa `google-github-actions/auth@v2` (`id-token: write`, gratis em repo privado) impersonando a SA `github-actions-deploy@mw-personal-trainer`; Node **24** no job; firebase-tools **15.22.3** instalado **localmente** com `overrides: { "google-auth-library": "^10.9.0" }` e rodado via `./node_modules/.bin/firebase`. O override e **obrigatorio**: o STS exchange do WIF roda dentro da `google-auth-library` e a `^9.x` bundled (gaxios 6) quebra com `Premature close`; `npx --package` ignora `overrides`. Sem billing, sem chave longa. **NAO** usar `--token`/`FIREBASE_TOKEN` (revogado de `login:ci`, prioridade sobre o GAC, mascara a credencial WIF). O secret `PT_DEPLOY_SA_B64` fica **so para rollback** (remocao das chaves = passo final pendente). **Validar deploy pelo `updateTime` da function (out-of-band)**, nao so pelo exit code (gen2 sai 0 com 409 transiente no retry). Infra WIF/IAM/historico: `security/firebase-wif-migration-plan-2026-06-26.md` e `security/firebase-deploy-fix-plan-2026-06-26.md`.
...
- Autenticacao de deploy: usa `google-github-actions/auth@v2` (WIF/OIDC keyless, ver nota acima).
...
- **Registro do SW** (`public/sw-register.js`, carregado por `index.html`): `register(..., { updateViaCache: "none" })` evita cache HTTP de 24h do proprio sw.js. Listener `message` dispara CustomEvent `pt:sw-updated` no window. O registro fica fora do HTML para permitir CSP enforcement sem `script-src 'unsafe-inline'`.
...
- **Revalidacao online (`public/sw-register.js`)**: `registration.update()` eh chamado em 4 gatilhos enquanto a aba esta online — `window.focus`, `document.visibilitychange` voltando a visivel, evento `online`, e `setInterval` de 5 min. Garante que aba aberta por horas/dias detecta nova versao sem reload manual. `updatefound` + `statechange === "installed"` no client dispara `pt:sw-updated` assim que o novo SW entra em `waiting`, sem esperar `activate`.
...
- **Arquivos criticos**: `public/manifest.json`, `public/sw.js`, `public/sw-register.js`, `public/icons/`, `scripts/generate-icons.mjs`, `vite.config.js` (plugin `pwaBaseTransform`), `index.html` (meta iOS + script externo do register), `src/hooks/useInstallPrompt.js`, `src/components/ui/InstallButton.jsx`, `src/components/ui/UpdateBanner.jsx`, `firebase.json` (headers).
