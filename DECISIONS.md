...
| `--only "hosting,firestore,functions:api,functions:claudeWorker"` no CI | Deploy seletivo nunca toca a função `gateway` que vive no sub-projeto `ai-gateway` (Firebase project compartilhado). Repete o mesmo padrão do deploy manual e protege contra a regressão histórica de 2026-05-08 |
